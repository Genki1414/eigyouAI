...
### T116. 本番の疎通確認と、フォーム検出ロジックの作り直し(実在82社で計測)(2026-09-20)
...
`form_navigator.py test`に27項目を追加(計65項目)。既存テストは全て通る
...
**未対応として残したもの**
- `page.evaluate()`にはPlaywright側のタイムアウトが無く、`navigate_and_submit()`にも
  1社あたりの上限時間が無い。実際、計測中に1社のページで**レンダラが11分以上返ってこない**
  事象が起きた。本番では`requeue_stale_running`(T96)が拾うまでワーカーが1本塞がる。
  1社あたりのデッドラインを入れるのが望ましい(今回は範囲を広げないため見送り)。
- `goto_failed`(到達不能)は今回の範囲外。国内プロキシ`FORM_PROXY_POOL`の契約待ち。

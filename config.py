...
# ── 契約キャンペーン(lp_hirakeru.html。T86) ──────────
# 4,000件/月プランの月額を、本契約の累計社数(hq.htmlで作成されたkind='client'の
# テナントのうちCAMPAIGN_START以降に作られたもの)に応じて段階的に決める。
# 契約時に決まった価格は、その会社の契約中ずっと据え置く(ユーザー判断 2026-09-17)。
# 何社目かは「本契約テナントの作成順」で数えるため、契約成立時にhq.htmlでテナントを
# 作る運用を守ること(申し込み<inquiries>の件数では数えない)。
CAMPAIGN_PLAN_LABEL = "ライト(4,000件/月)"
CAMPAIGN_REGULAR_PRICE_YEN = 40000
CAMPAIGN_TIERS = [(50, 10000), (100, 20000), (150, 30000)]   # (累計社数の上限, 月額・円)
CAMPAIGN_START = "2026-09-17"

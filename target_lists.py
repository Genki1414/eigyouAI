...
def send_list(con, tenant_id, list_id, subject, body, dry_run=True, track_clicks=False,
    ...
    必須ではない。空のまま呼ぶと、その商材のAI生成文面(products.generate_message()。
    無ければこの時点で1回だけ生成してtenant_products.ai_subject/ai_bodyへ
    キャッシュする)を使う。指定すればテナントの手入力を優先する(手動上書き)。
    いずれの場合も、以後は通常のリストと全く同じ経路(1本の件名・本文を全社へ
    マージタグ<##TO_COMPANY_NAME##等>置換して送る)で送信される — 会社ごとの
    個別化はまだ行わない(products.pyモジュールdocstring参照)。
    product_idを持たない通常のリストでは従来通りこの引数が必須(呼び出し元
    <api.py>で検証する)。
    """
    import senders
    import db

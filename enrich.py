...
コスト実測(2026-08、資本金300万〜1億円のサンプルの実測平均。旧見積もりの「1社2〜4円」は
実態と大きく乖離していたため実測値に置き換え。web検索max_uses=2 + effort=lowが効いた):
  同期実行  1社あたり入力2.0万トークン/出力465トークン/web検索2.0回
            ≈ $0.06〜$0.09 ≈ 8.9〜13.0円
  --batch   Message Batches APIはトークン課金が半額(web検索の$10/1000件分は対象外)。
            同じ使用量なら ≈ $0.04〜$0.05 ≈ 6.3〜8.0円。処理は非同期(最大24時間、
            通常1時間以内)になるが、この用途はリアルタイム性が不要なため実質デメリットなし。
            事前絞込3,000社なら概算$127〜160(--batchなしの$196〜262から約35%減)。
  検証済みで不採用: web_search_20260209(動的フィルタリング) — コード実行を
            介する分のオーバーヘッドで逆に入力トークンが2〜3倍に悪化(実測)。
...
def build_request(row, model=MODEL):
    ...
    effort="low": 実測で出力トークンが平均965→465(約52%減)、コストのみならず
    「thinkingがmax_tokensを圧迫してJSON途中で切れる」リスクも下げた
    (6社の実測でJSON抽出失敗0件・is_target_business等の判定品質も劣化なしを確認)。

    web_search_20260209(動的フィルタリング)は逆に悪化したため不採用: 実測で
    入力41,268〜68,130トークン(コード実行を介する分のオーバーヘッドが、この
    タスクの検索結果量では削減分を上回った)。素朴なweb_search_20250305のまま。
    """
    return dict(

...
REVIEW_BINS = [(0, 1), (1, 6), (6, 21), (21, 51), (51, 101), (101, None)]
...
def main():
    ...
    rows = [dict(r) for r in con.execute("""
        SELECT id, name, pref, city, address, trades, capital,
               has_website, website_url, website_quality, hiring_now,
               est_employees, google_reviews, prime_ratio, enrich_note, enriched_at
    ...
    n = len(rows)
    fields = ["has_website", "website_url", "website_quality", "hiring_now",
              "est_employees", "google_reviews", "prime_ratio", "enrich_note"]
    ...
    stats = {
        ...
        "review_hist": histogram(rows, "google_reviews", REVIEW_BINS),
...
<main>
  ...
  <div class="grid2">
    ...
    <div class="panel"><h3>Googleレビュー件数 (google_reviews)</h3><div id="reviewHist"></div></div>
...
barList("reviewHist", S.review_hist);
...
document.getElementById("tbl").innerHTML = `
  <tr><th>会社名</th><th>所在地</th><th>業種</th><th class="num">資本金(千円)</th>
      <th>HP</th><th>URL</th><th class="num">品質</th><th>求人</th>
      <th class="num">従業員数</th><th class="num">レビュー</th><th class="num">元請比率</th><th>所見</th></tr>` +
  rows.map(r => `<tr>
    <td><b>${r.name}</b></td>
    <td>${(r.pref||"")+(r.city||"")}</td>
    ...
    <td class="num">${r.google_reviews ?? "-"}</td>
